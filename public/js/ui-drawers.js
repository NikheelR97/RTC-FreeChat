...
export function toggleRightDrawer(targetDrawerId) {
    ...
    const isTargetOpen = !targetEl.classList.contains('hidden') && document.body.classList.contains('drawer-open-right');
...
function renderGallery() {
    ...
    if (currentGalleryTab === 'images') {
        filtered = attachments.filter(m => m.attachment.mimeType && m.attachment.mimeType.startsWith('image/'));
...
if (elements.closeGalleryBtn) elements.closeGalleryBtn.onclick = () => elements.galleryDrawer.classList.add('hidden');

...
export function createPeerConnection(peerId, remoteDisplayName, shouldInitiate) {
  ...
  pc.onnegotiationneeded = async () => {
    if (shouldInitiate) {
      // Only the polite peer (initiator) should restart negotiation?
...
function checkPeerVideoStatus(peerId, pc) {
  ...
  const hasActiveVideo = receivers.some(
    (r) => r.track.kind === 'video' && !r.track.muted && r.track.readyState === 'live'
  );

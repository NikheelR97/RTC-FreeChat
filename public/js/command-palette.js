...
export function setupCommandPalette(switchChannel) {
  ...
  function filterCommands(query) {
    ...
    if (!query) {
      ...
      filteredCommands = all.filter(
        (c) =>
          c.label.toLowerCase().includes(lower) || (c.sub && c.sub.toLowerCase().includes(lower))
      );
    }
    selectedIndex = 0;
    render();
  ...
  function render() {
    ...
    if (filteredCommands.length === 0) {
      resultsContainer.innerHTML =
        '<div style="padding:16px;text-align:center;color:var(--text-muted)">No results found</div>';
      return;
    }

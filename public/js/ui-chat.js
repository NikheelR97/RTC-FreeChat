...
export function appendChatMessage({ id, socketId, displayName, text, timestamp, attachment, isOwnMessage, reactions, replies, replyCount }) {
    if (!elements.chatMessages) return;
    const atBottom =
        Math.abs(elements.chatMessages.scrollHeight - elements.chatMessages.scrollTop - elements.chatMessages.clientHeight) < 50;

    const time = timestamp ? new Date(timestamp) : new Date();

    // Grouping
    const isSameSender = lastMessage && lastMessage.socketId === socketId && lastMessage.displayName === displayName;
    ...
    if (attachment && attachment.url) {
        if (attachment.mimeType && attachment.mimeType.startsWith('image/')) {
            ...
            const sizeStr = attachment.size ? ` <span style="font-size: 10px; opacity: 0.7;">(${(attachment.size / 1024).toFixed(1)} KB)</span>` : '';

...
export function setupGifPicker() {
    ...
    elements.gifPickerContent.addEventListener('scroll', () => {
        if (isLoading) return;
        if (elements.gifPickerContent.scrollTop + elements.gifPickerContent.clientHeight >= elements.gifPickerContent.scrollHeight - 50) {
            if (currentSearch) {
...
async function fetchTrendingGifs(append = false) {
    ...
    if (!append) elements.gifPickerContent.innerHTML = '<div class="loading">Loading trending GIFs...</div>';
    ...
    try {
        ...
        if (!append) elements.gifPickerContent.innerHTML = '<div class="error">Failed to load GIFs</div>';
    } finally {
        isLoading = false;
    }
}
...
async function searchGifs(query, append = false) {
    ...
    if (!append) elements.gifPickerContent.innerHTML = `<div class="loading">Searching for "${query}"...</div>`;
    ...
    try {
        ...
        if (!append) elements.gifPickerContent.innerHTML = '<div class="error">Failed to search GIFs</div>';
    } finally {
        isLoading = false;
    }
}
